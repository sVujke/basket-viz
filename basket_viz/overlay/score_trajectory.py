...
# # player_counts = df[self.subject_col].value_counts()
# # df = df[df[self.subject_col].isin(player_counts[player_counts >= 200].index)]
...
#     for player in df[self.subject_col].unique():
#         if player not in selected_players:
#             player_data = df[df[self.subject_col] == player]
...
#         self.ani = None
#         self.subject_col = "PLAYER"
#         self.vlines = {"color": "black", "linestyle": "--"}
#         self.hlines = {"color": "black", "linestyle": "--"}
#         self.xlabel_params = {"fontsize": 18, "labelpad": 25}
#         self.ylabel_params = {"fontsize": 18, "labelpad": 25}
#         self.title_params = {"fontsize": 20}
#         self.item_line_params = {"linewidth": 4}
#         self.background_line_params = {"color": "gray", "alpha": 0.3}
#         self.vlines_text_params = {
#             "ha": "center",
#             "va": "top",
#             "color": "black",
#             "fontsize": 12,
#         }
#         self.hlines_text_params = {
#             "ha": "left",
#             "va": "center",
#             "color": "black",
#             "fontsize": 12,
...
#         # Plot all players in gray with low opacity
#         for item in df[self.subject_col].unique():
#             if item not in selected_items:
#                 item_data = df[df[self.subject_col] == item]
#                 self.plot_trajectory_data(
#                     item_data, column, **self.background_line_params
#                 )
...
#             self.plot_trajectory_data(
#                 item_data, column, color=color, **self.item_line_params, label=item
#             )

#         if v_lines:
#             for vline, label in v_lines.items():
#                 plt.axvline(x=vline, **self.vlines)
#                 plt.text(vline, plt.ylim()[0] - 0.1, label, **self.vlines_text_params)

#         if h_lines:
#             for hline, label in h_lines.items():
#                 plt.axhline(y=hline, **self.hlines)
#                 plt.text(plt.xlim()[0] - 0.1, hline, label, **self.hlines_text_params)

#         plt.xlabel(xlabel, **self.xlabel_params)
#         plt.ylabel(ylabel, **self.ylabel_params)
#         plt.title(title, **self.title_params)
#         plt.grid(self.grid)
