...
class ShotChart:
    ...
    def set_config_param(self, **kwargs):
        for key, value in kwargs.items():
            if key in self.config:
                ...
                raise KeyError(f"Config parameter '{key}' is not a valid config option.")
    ...
    def plot_field_goal_scatter(self, made, miss, title=None):
        ...
        if self.config['plot_shots'] in ['all', 'made']:
            ax.plot(made[coord_x], made[coord_y], 'o', label='Made', color=self.config['color_map']['made'], markersize=self.config['marker_size'])
        
        if self.config['plot_shots'] in ['all', 'miss']:
            ax.plot(miss[coord_x], miss[coord_y], 'x', label='Missed', color=self.config['color_map']['miss'], markersize=self.config['marker_size'])
    ...
    def _create_court_elements(self):
        ...
        corner_three_a, corner_three_b, three_arc = self._create_three_point_line(color, lw)
        center_outer_arc = self._create_center_court(color, lw)
        outer_lines = self._create_outer_lines(color, lw) if self.config['outer_lines'] else None
    ...
    def _create_restricted_zone(self, color, lw):
        return Arc((0, 0), 2 * 125, 2 * 125, theta1=0, theta2=180, linewidth=lw, color=color)
    ...
    def _create_center_court(self, color, lw):
        return Arc((0, 1400 - 157.5), 2 * 180, 2 * 180, theta1=180, theta2=0, linewidth=lw, color=color)

    def _create_outer_lines(self, color, lw):
        return Rectangle((-750, -157.5), 1500, 1400, linewidth=lw, color=color, fill=False)
